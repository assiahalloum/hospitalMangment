...
class Admin extends CI_Controller
{
	...
	/***Manage pharmacists**/
	function manage_human_resources($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('admin_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
		...
		if ($param1 == 'create') {
			...
			$this->session->set_flashdata('flash_message', ('Account Opened'));
			redirect(base_url() . 'index.php?admin/manage_human_resources', 'refresh');
		}
		if ($param1 == 'edit' && $param2 == 'do_update') {
			$data['name']     = $this->input->post('name');
			$data['email']    = $this->input->post('email');
		...
		$page_data['page_name']   = 'manage_human_resources';
		$page_data['page_title']  = ('Manage RH');
